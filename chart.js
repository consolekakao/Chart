...
async function botLog() {
  ...
  for (let i = 0; i < data.data.length; i++) {
    ele.innerHTML += `<tr>
            <td id="botLogtime">${decodeURI(
              data.data[i].time
            )}</td><td>${decodeURI(
      data.data[i].servername
    )}</td><td>${decodeURI(data.data[i].channelname)}</td>
            <td>${decodeURI(data.data[i].usernick)}</td><td>${decodeURI(
      data.data[i].usecommand
    )}</td><td>${decodeURI(data.data[i].status)}</td>
            </tr>`;
  }
}
